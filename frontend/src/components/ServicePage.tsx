    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-100 text-slate-900 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-20 md:py-24 relative z-10">
        ...
        <header className="text-center mb-10 sm:mb-16 max-w-4xl mx-auto">
          ...
          {/* Stats Cards */}
          {stats.length > 0 && (
            <div className="mt-8 sm:mt-14 grid grid-cols-3 gap-2 sm:gap-4 max-w-3xl mx-auto">
              {stats.map((s, i) => (
                <div
                  key={i}
                  className="rounded-lg sm:rounded-2xl bg-slate-50 border border-slate-100 p-2 sm:p-4 text-center shadow-none"
                >
                  <p className="text-xs sm:text-base font-semibold text-teal-600 mb-0.5 sm:mb-1">{s.value}</p>
                  <p className="text-xs text-slate-500 font-medium line-clamp-2">{s.label}</p>
                </div>
              ))}
            </div>
          )}
              ...
              <div className="mb-8 sm:mb-14">
                ...
                <ul className="space-y-3 sm:space-y-4">
                  {data.materials.map((m, i) => (
                    <li key={i} className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-slate-500 leading-relaxed">
                      <svg className="w-4 sm:w-5 h-4 sm:h-5 text-slate-300 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                  ...
                  <ul className="space-y-3 sm:space-y-4">
                    {data.collateral && data.collateral.map((c, i) => (
                      <li key={`collateral-${i}`} className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-slate-500 leading-relaxed">
                        <svg className="w-4 sm:w-5 h-4 sm:h-5 text-slate-300 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                        <span>{c}</span>
                    ...
                    {data.conditions && data.conditions.map((c, i) => (
                      <li key={`condition-${i}`} className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-slate-500 leading-relaxed">
                        <svg className="w-4 sm:w-5 h-4 sm:h-5 text-slate-300 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                        <span>{c}</span>
