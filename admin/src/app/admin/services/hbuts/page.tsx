...
export default function HbutsServiceAdminPage() {
  ...
  const handleServerSubmit = async () => {
    ...
    try {
      const response = await fetch('/api/admin/services/hbuts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      await response.json()
      alert(' Худалдаа, бизнес, үйлчилгээ, туризмын үйлчилгээг серверт амжилттай илгээлээ')
    } catch (error) {
          ...
          action={
            <div className="flex gap-3">
              <button
                ...
                className={`px-4 py-2.5 text-sm font-semibold rounded text-white transition-colors ${
                  isSavingToServer
              ...
              <SaveResetButtons onSave={handleLocalSave} onReset={handleReset} confirmMessage="Та хадгалахдаа итгэлтэй байна уу?" />
            </div>
          }
        />
